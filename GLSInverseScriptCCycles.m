function [sgrid,sprobs, rsgrid,optweightfromAIC,t,propdata,sweightedsol,cdfS,cdfR]= GLSInverseScriptCCycles(sgrid,sprobs,rho,k,y0,propdata,tspan,disttype,figures)
...
points=length(sgrid);
    ...
    for i=1:length(rpointsvec)
        ...
        [gls_optpar,~,AIC_GLS,~,rsgrid]=GLSInverseFnCCycles(rho,k,y0,tspan,rpointsvec(i),propdata);
    ...
    [optweightfromAIC,~,~,weightedsol,rsgrid]=GLSInverseFnCCycles(rho,k,y0,tspan,optrpoints,propdata);
    % optweightfromAIC
    % rsgrid
    %keyboard
    ...
    elseif strcmp(disttype,'OnePoint') == 1 || strcmp(disttype,'TwoPoints') ==1
        ...
        cdfR=cumsum(optweightfromAIC);
        cdfS=cumsum(sprobs)./sum(sprobs);
    end
