...
elseif string(disttype) == 'NormalDiscrete'
    mu = .5; 
    sigma = .09;
    sprobs = pdf('Normal',sgrid,mu,sigma); 
...
%     sprobs = normpdf(sgrid,mu,sigma); 
 elseif string(disttype) == 'Bigaussian' %this should be cts 
     ...
     % sigma2=.1;
     % sigma1=.05;
     % weight2=.1; %typically use this pair %normally .9
     % weight1=.9; %normally .1
 ...
 % % % %new fun guy
     ...
     sigma2=.1;
     sigma1=.05;
     weight2=.99; %typically use this pair %normally .9
     weight1=.01; %normally .1
...
elseif string(disttype) == 'OnePoint'
    ...
    sprobs=sprobs';
    sprobs = sprobs/(sum(sprobs));
elseif string(disttype) == 'OnePointAlmost'
    pointval1=0.3; %normally .3
    %pointval2=0.8;
    prop1=0.99; %normally 0.9
    prop2=(1 - prop1)./(length(sgrid - 1));
    sprobs=ones(length(sgrid),1)*prop2;
